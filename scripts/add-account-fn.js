...
function switchAccountTab(tab) {
  ...
  const btn = document.querySelector('#sidebarNav button[data-tab="' + tab + '"]');
...
function renderAccountSettings(container) {
  ...
  container.innerHTML = '<div style="max-width:500px">' +
    ...
    '<div style="background:var(--white);border:1px solid var(--gray-200);border-radius:12px;padding:24px">' +
      ...
      '<div style="margin-bottom:16px"><label style="font-size:13px;color:var(--gray-500);display:block;margin-bottom:4px">Member since</label><div style="font-size:16px;font-weight:500">' + (user.createdAt ? new Date(user.createdAt).toLocaleDateString() : '—') + '</div></div>' +
    '</div>' +
    '<div style="margin-top:20px;display:flex;gap:10px">' +
      '<a href="/" style="padding:10px 24px;background:var(--primary);color:white;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px">Back to Search</a>' +
    '</div>' +
  '</div>';
}
`;

s = s.replace('function renderDashboardFavorites() {', newFns + '\nfunction renderDashboardFavorites() {');
